...
{if(typeof $(this).data('qtip')==='object'&&$(this).data('qtip'));};obj=new qTip($(this),config,id);$.fn.qtip.interfaces[id]=obj;if($.isPlainObject($(this).data('qtip')))
...
{$(this).css({opacity:''});if(self.options.position.type!=='static')self.focus();self.onShow.call(self,event);if($.browser.msie)self.elements.tooltip.get(0).style.removeAttribute('filter');};self.cache.toggle=1;if(self.options.position.type!=='static')
...
{zoom.css({zoom:1});if(self.elements.bgiframe)self.elements.bgiframe.width(newWidth).height(self.getDimensions.height);};return $.fn.qtip.log.error.call(self,1,$.fn.qtip.constants.EVENT_WIDTH_UPDATED,'updateWidth');},updateStyle:function(name)
...
positionTip.call(self);};};self.updateWidth();if(reposition!==false)
{if(self.options.position.type!=='static')
self.updatePosition(self.elements.tooltip.is(':visible'),true);if(self.options.style.tip.corner!==false)
...
{inactiveEvents=['click','dblclick','mousedown','mouseup','mousemove','mouseout','mouseenter','mouseleave','mouseover','touchstart'];function inactiveMethod(event)
